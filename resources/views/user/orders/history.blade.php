        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                        ...
                        <div class="p-6">
                            <div class="flex items-center justify-between">
                                <div>
                                    ...
                                    <p class="text-3xl font-bold text-green-600">{{ $orders->where('status', 'paid')->count() }}</p>
                                </div>
                        ...
                        <div class="p-6">
                            <div class="flex items-center justify-between">
                                <div>
                                    ...
                                    <p class="text-2xl font-bold text-purple-600">Rp {{ number_format($orders->where('status', 'paid')->sum('total'), 0, ',', '.') }}</p>
                                </div>
                            ...
                            <div class="flex items-center justify-between">
                                <div>
                                    ...
                                    <p class="text-2xl font-bold text-blue-600">
                                        Rp {{ $orders->where('status', 'paid')->count() > 0 ? number_format($orders->where('status', 'paid')->avg('total'), 0, ',', '.') : '0' }}
                                    </p>
                                </div>
                                ...
                                <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
                                        ...
                                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 text-sm text-gray-600 mb-4">
                                            <div class="flex items-center">
                                                <svg class="w-4 h-4 mr-2 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3a1 1 0 011-1h6a1 1 0 011 1v4h3a2 2 0 012 2v1a1 1 0 01-1 1H10a1 1 0 01-1-1V9a2 2 0 012-2h3zM8 7H6a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2V9a2 2 0 00-2-2h-2"></path>
                                                </svg>
                                                ...
                                                <div class="space-y-1">
                                                    @foreach($order->items->take(3) as $item)
                                                        <div class="flex justify-between items-center text-sm">
                                                            <span class="text-gray-600">{{ $item->nama_produk ?? 'Product' }}</span>
                                                            <span class="text-gray-500">{{ $item->quantity }}x - Rp {{ number_format($item->harga * $item->quantity, 0, ',', '.') }}</span>
                                                        </div>
                                                    @endforeach
                                    ...
                                    <div class="lg:text-right lg:ml-6">
                                        ...
                                        <div class="flex lg:flex-col space-x-2 lg:space-x-0 lg:space-y-2">
                                            ...
                                            @if($order->status == 'paid')
                                                <button class="flex-1 lg:flex-none px-4 py-2 bg-green-100 hover:bg-green-200 text-green-700 text-sm font-medium rounded-lg transition-colors duration-200">
                                                    Download Invoice
                                                </button>
                                            @endif
                                        </div>
                                            ...
                                            <div class="space-y-3">
                                                @foreach($order->items as $item)
                                                    <div class="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                                                        <div>
                                                            <p class="font-medium text-gray-900">{{ $item->nama_produk ?? 'Product' }}</p>
                                                            <p class="text-sm text-gray-500">Quantity: {{ $item->quantity }}</p>
                                                            <p class="text-sm text-gray-500">Unit Price: Rp {{ number_format($item->harga, 0, ',', '.') }}</p>
                                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <div class="bg-gray-50 rounded-lg p-4 space-y-2">
                                                <div class="flex justify-between text-sm">
                                                    ...
                                                    <span class="font-medium">{{ \Carbon\Carbon::parse($order->tanggal)->format('d F Y, H:i') }}</span>
                                                </div>
                                                <div class="flex justify-between text-sm">
                                                    <span class="text-gray-600">Transaction Code:</span>
                                                    <span class="font-mono font-medium">{{ $order->kode_trx_jual }}</span>
                                                </div>
                                                <div class="flex justify-between text-sm">
            ...
            @else
                ...
                <div class="text-center py-16">
                    ...
                    <h3 class="text-2xl font-bold text-gray-900 mb-2">No Order History</h3>
                    <p class="text-gray-500 mb-8 max-w-md mx-auto">You haven't completed any orders yet. Start shopping to build your order history!</p>
                    <div class="space-y-3 sm:space-y-0 sm:space-x-3 sm:flex sm:justify-center">
                        <a href="{{ route('product.index') }}" 
                           class="inline-flex items-center bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white font-semibold py-3 px-6 rounded-xl shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all duration-200">
