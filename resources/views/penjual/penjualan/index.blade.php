  ...
  <div class="flex">
    ...
    <main class="flex-1 p-5">
      ...
      <div class="mb-3 flex space-x-3">
        <a href="/pembelian" class="bg-green-600 text-white text-xs sm:text-sm font-semibold px-3 py-1 rounded-sm hover:bg-green-700 transition">
            Input Penjualan
          </a>
        <a href="/data_beli" class="bg-green-600 text-white text-xs sm:text-sm font-semibold px-3 py-1 rounded-sm hover:bg-green-700 transition">
        ...
        <fieldset class="border border-gray-400 p-3 rounded-sm mb-3">
          ...
          <div class="flex flex-col sm:flex-row sm:space-x-3">
            ...
            <div class="flex-1 grid grid-cols-[auto_1fr] gap-x-2 gap-y-2 text-xs sm:text-sm">
                <label for="noNota" class="text-gray-800 pt-1">No. Nota</label>
                <input id="noNota" type="text" value="{{ $lastKodeHariIni }}" readonly class="bg-gray-200 border border-gray-300 rounded-sm px-2 py-1 text-xs sm:text-sm w-[150px]" />

                <label for="supplier_id" class="text-gray-800 pt-1">Pembeli</label>
                <select id="supplier_id" name="supplier_id" class="border border-gray-300 rounded-sm px-2 py-1 text-xs sm:text-sm w-[150px]">
                ...
                <input id="tanggal" type="date" value="{{ now()->format('Y-m-d') }}" class="border border-gray-300 rounded-sm px-2 py-1 text-xs sm:text-sm w-[120px]" />
            </div>

            <!-- Box Tagihan -->
            <div class="flex-1 border border-gray-400 rounded-sm ml-0 sm:ml-3 mt-3 sm:mt-0 flex items-center justify-center">
...
<form action="{{ route('pembelian.addItem') }}" method="POST" class="mt-3">
    @csrf
    <div class="grid grid-cols-1 sm:grid-cols-[110px_180px_70px_120px_90px_110px] gap-2 text-xs sm:text-sm items-center">
      <!-- Kode Barang -->
      <select name="id_produk" class="border border-gray-300 rounded-sm px-2 py-1 w-full" required onchange="isiNamaBarang(this)">
      ...
      <div class="flex items-center space-x-1">
        ...
        <input type="text" id="harga_satuan" name="harga_satuan" placeholder="Harga Satuan"
          class="border border-gray-300 rounded-sm px-2 py-1 w-full text-xs sm:text-sm bg-gray-100" readonly required />
      </div>
      ...
      <div class="flex items-center space-x-1">
        ...
        <input type="text" id="harga_akhir" placeholder="Harga Akhir"
          class="border border-gray-300 rounded-sm px-2 py-1 w-full text-xs sm:text-sm bg-gray-100" readonly />
      </div>
    </div>
    <div class="flex items-end mt-2">
      <button type="submit" class="bg-orange-600 text-white text-xs sm:text-sm font-semibold px-3 py-1 rounded-sm hover:bg-orange-700 transition">
        Tambahkan Data
      </button>
    </div>
  </form>
        ...
        <tbody>
            ...
            @foreach(session('cart_pembelian', []) as $item)
              ...
              <tr class="border border-blue-500">
                <td class="border border-blue-500 px-2 py-1 text-center font-mono text-[10px] sm:text-xs">{{ $item['id_produk'] }}</td>
                <td class="border border-blue-500 px-2 py-1">{{ $item['nama_produk'] }}</td>
                <td class="border border-blue-500 px-2 py-1 text-center font-mono">Rp {{ number_format($item['harga_satuan'], 0, ',', '.') }}</td>
                <td class="border border-blue-500 px-2 py-1 text-center">{{ $item['jumlah'] }}</td>
                <td class="border border-blue-500 px-2 py-1 text-center font-mono">Rp {{ number_format($harga_akhir, 0, ',', '.') }}</td>
                <td class="border border-blue-500 px-2 py-1 text-center">
                  <form action="{{ route('pembelian.removeItem', $item['id_produk']) }}" method="POST">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="bg-red-600 text-white text-xs px-2 py-1 rounded">Hapus</button>
                  </form>
                </td>
              </tr>
            @endforeach
  ...
  <div class="flex flex-col sm:flex-row sm:space-x-6 text-xs sm:text-sm">
    <div class="border border-gray-400 p-3 rounded-sm w-full sm:w-[350px] mb-3 sm:mb-0">
      ...
      <div class="flex items-center mb-2 space-x-1">
        ...
        <input type="text"
               id="diskon_rupiah"
               value="{{ number_format($diskon_rupiah, 2, ',', '.') }}"
               readonly
               class="border border-gray-300 rounded-sm px-2 py-1 w-[110px] bg-gray-200 font-mono text-right" />
      </div>
      ...
      <div class="flex items-center">
        ...
        <input type="text"
          ...
          <!-- Kanan -->
        <div class="flex-1 border border-gray-400 p-3 rounded-sm grid grid-cols-[auto_1fr] gap-x-2 gap-y-2 items-center text-xs sm:text-sm sm:w-[350px]">
            <label class="text-right font-semibold">Bayar</label>
            <div class="flex items-center space-x-1">
                <span class="font-mono">Rp.</span>
                <input id="bayar" type="text" class="border border-gray-300 rounded-sm px-2 py-1 w-full font-mono text-right" oninput="hitungKembalian()" />
            </div>
            <label class="text-right font-semibold">Kembalian</label>
            <div class="flex items-center space-x-1">
                <span class="font-mono">Rp.</span>
                <input id="kembalian" type="text" readonly class="border border-gray-300 rounded-sm px-2 py-1 w-full bg-gray-200 font-mono text-right" />
            </div>

            </div>
            <div class="mt-3 flex justify-center space-x-3">
                <button type="submit" class="bg-orange-600 text-white font-semibold px-4 py-1 rounded-sm hover:bg-orange-700 transition text-sm">Simpan</button>
                <button type="reset" class="bg-orange-600 text-white font-semibold px-4 py-1 rounded-sm hover:bg-orange-700 transition text-sm">Batal</button>
            </div>
        </div>
      </form>
    </main>
