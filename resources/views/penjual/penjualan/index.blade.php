...
<head>
    ...
    @php
        ...
        // Perhitungan diskon dan total awal (akan diupdate oleh JS)
        $diskon_persen = old('diskon') ?? 0;
        $diskon_rupiah = ($diskon_persen / 100) * $subtotal;
        $total_final = $subtotal - $diskon_rupiah;
    @endphp
    <style>
        .sidebar {
            width: 220px;
...
<body class="bg-gradient-to-br from-slate-100 via-slate-200 to-slate-100 min-h-screen font-sans">
            ...
            <fieldset class="border border-gray-400 p-3 rounded-sm mb-3">
                ...
                <div class="flex flex-col sm:flex-row sm:space-x-3">
                    ...
                    <div class="flex-1 grid grid-cols-[auto_1fr] gap-x-2 gap-y-2 text-xs sm:text-sm">
                        ...
                        <input id="noNota" type="text" value="{{ $lastKodeHariIni }}" readonly
                            class="bg-gray-200 border border-gray-300 rounded-sm px-2 py-1 text-xs sm:text-sm w-[150px]" />

                        <label for="id_pembeli" class="text-gray-800 pt-1">Pembeli</label>
                    ...
                    <div
                        ...
                        <p id="tagihan" class="text-red-600 font-bold text-lg sm:text-xl">
                            Tagihan : Rp. {{ number_format($total_final, 2, ',', '.') }}
                        </p>
                    </div>
                </div>
                </form>
                <!-- FORM TAMBAH ITEM DIPISAH -->
                <form action="{{ route('penjualan.addItem') }}" method="POST" class="mt-3">
                    @csrf
                    <div
                        ...
                        <input type="number" name="jumlah" id="jumlah" min="1" placeholder="Jumlah Beli"
                            class="border border-gray-300 rounded-sm px-2 py-1 w-full" required
                            oninput="hitungTotal()" />
                ...
                <fieldset class="border border-gray-400 p-0 rounded-sm mb-3">
                    ...
                    <div class="overflow-x-auto scrollbar-thin">
                        <table class="w-full border-collapse border border-blue-500 text-xs sm:text-sm">
                            ...
                            <tbody>
                                @php $total = 0; @endphp
                                @foreach (session('cart_penjualan', []) as $item)
                                    @php
                ...
                <button type="reset"
                    class="bg-orange-600 text-white font-semibold px-4 py-1 rounded-sm hover:bg-orange-700 transition text-sm">Batal</button>
            </div> --}}
                <form action="{{ route('penjualan.store') }}" method="POST" class="mt-4">
                    @csrf
                    {{-- Input tersembunyi untuk data transaksi --}}
                    <input type="hidden" name="kode_trx_jual" value="{{ $lastKodeHariIni ?? '' }}">

                    {{-- Input tersembunyi untuk Diskon dan Total Final --}}
                    <input type="hidden" name="diskon" id="diskon_final_input" value="{{ old('diskon', 0) }}">
                    <input type="hidden" name="total_final" id="total_final_input" value="{{ $total }}">

                    <div class="flex items-center space-x-3">
                        <button type="submit"
                            class="bg-blue-600 text-white font-semibold px-4 py-2 rounded-md hover:bg-blue-700 transition text-sm">
                            <i class="fas fa-save mr-1"></i> Simpan Transaksi
                        </button>

                        <a href="{{ route('penjualan.clearAll') }}"
                            class="bg-red-600 text-white font-semibold px-4 py-2 rounded-md hover:bg-red-700 transition text-sm">
    ...
    <script>
        ...
        function hitungDiskon() {
            ...
            document.getElementById('diskon_rupiah').value = formatAngka(diskonRupiah);
            document.getElementById('total_harga').value = formatAngka(total);

            // Update hidden input values for submission
            document.getElementById('diskon_final_input').value = diskonPersen;
            document.getElementById('total_final_input').value = total;

            updateTagihan(total);
            hitungKembalian(); // Hitung ulang kembalian jika diskon berubah
        }
        ...
        function updateTagihan(total) {
            document.getElementById('tagihan').textContent = 'Tagihan : Rp ' + formatAngka(total);
        }
        ...
        function hitungTotal() {
            ...
            document.getElementById('harga_akhir').value = totalItem;
        }

        // Fungsi untuk menghitung kembalian
        function hitungKembalian() {
            const bayarString = document.getElementById('bayar').value.replace(/\./g, '').replace(',', '.') || '0';
            const bayar = parseFloat(bayarString);

            // Ambil total dari input tersembunyi yang sudah diupdate
            const total = parseFloat(document.getElementById('total_final_input').value) || 0;

            const kembalian = bayar - total;
            document.getElementById('kembalian').value = (kembalian >= 0) ? formatAngka(kembalian) : formatAngka(0);
        }
        ...
        document.getElementById('id_pembeli').addEventListener('change', function() {
            const idpembeli = this.value;

            fetch("{{ route('penjualan.setPembeli') }}", {
                    method: 'POST',
                    headers: {
                ...
                .then(response => {
                    if (!response.ok) {
                        throw new Error('Gagal menyimpan pembeli.');
                    }
                    return response.json();
                })
                .then(data => {
                    console.log('Pembeli disimpan:', data.message);
                    // alert('Pembeli berhasil disimpan.');
                })
                .catch(error => {
                    console.error('Gagal menyimpan pembeli:', error);
                    alert('Terjadi kesalahan saat menyimpan pembeli. Silakan coba lagi.');
                });
        });
    </script>
