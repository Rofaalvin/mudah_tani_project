...
@section('content')
    <div class="container mt-4">
        ...
        <div class="card shadow-sm mb-3">
            <div class="card-body">
                <form action="{{ route('penjual.index') }}" method="GET" class="d-flex">
                    <input type="search" name="search" class="form-control me-2"
                        placeholder="Cari penjual berdasarkan nama, email, atau no hp..." value="{{ $search ?? '' }}">
                    <button class="btn btn-primary" type="submit">Cari</button>
                    <a href="{{ route('penjual.index') }}" class="btn btn-secondary ms-2">Reset</a>
                </form>
            </div>
        </div>
        ...
        <table class="table table-bordered">
            ...
            <tbody>
                @forelse ($penjuals as $penjual)
                    <tr>
                        ...
                        <td>
                            ...
                            <button class="btn btn-warning btn-sm" data-bs-toggle="modal"
                                data-bs-target="#editPenjualModal{{ $penjual->id }}">
                                Edit
                            </button>

                            <!-- Form Hapus -->
                            <form action="{{ route('penjual.destroy', $penjual->id) }}" method="POST" class="d-inline"
                    ...
                    <!-- Modal Edit Penjual -->
                    <div class="modal fade" id="editPenjualModal{{ $penjual->id }}" tabindex="-1"
                        aria-labelledby="editPenjualModalLabel" aria-hidden="true">
                                    ...
                                    <form action="{{ route('penjual.update', $penjual->id) }}" method="POST">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <input type="text" class="form-control" name="id_penjual"
                                                value="{{ $penjual->id }}" readonly>
                                        </div>
                                        <div class="mb-3">
                                            <label class="form-label">Nama</label>
                                            <input type="text" class="form-control" name="name"
                                                value="{{ $penjual->name }}" required>
                                        </div>
                                        <div class="mb-3">
                                            <label class="form-label">Email</label>
                                            <input type="email" class="form-control" name="email"
                                                value="{{ $penjual->email }}" required>
                                        </div>
                                        <div class="mb-3">
                ...
                @empty
                    <tr>
                        <td colspan="6" class="text-center">
                            @if (!empty($search))
                                Penjual dengan kata kunci "{{ $search }}" tidak ditemukan.
                            @else
    ...
    <div class="modal fade" id="tambahPenjualModal" tabindex="-1" aria-labelledby="tambahPenjualModalLabel"
        aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
